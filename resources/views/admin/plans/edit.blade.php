...
.pf-input, .pf-select, .pf-textarea { width: 100%; padding: 10px 13px; border: 1.5px solid var(--border); border-radius: 9px; background: var(--surface); color: var(--text1); font-size: .875rem; font-family: inherit; outline: none; transition: border-color .2s; box-sizing: border-box; -webkit-appearance: none; }
...
<div class="panel plan-form-wrap">
    <div class="panel-title" style="margin-bottom:22px;"><i class="fas fa-edit"></i> Edit Plan: {{ $plan->name }}</div>
    ...
    <form method="POST" action="{{ route('admin.plans.update', $plan) }}">
        ...
        <div class="pf-grid">
            <div class="pf-field">
                ...
                <input type="text" name="name" class="pf-input" value="{{ old('name', $plan->name) }}" required>
            </div>
            <div class="pf-field">
                <label class="pf-label">Price (BDT) <span>*</span></label>
                <input type="number" name="price" class="pf-input" value="{{ old('price', $plan->price) }}" min="0" required>
            </div>
            <div class="pf-field">
                <label class="pf-label">Duration (days) <span>*</span></label>
                <input type="number" name="duration_days" class="pf-input" value="{{ old('duration_days', $plan->duration_days) }}" min="1" required>
            </div>
            <div class="pf-field">
                <label class="pf-label">Sort Order</label>
                <input type="number" name="sort_order" class="pf-input" value="{{ old('sort_order', $plan->sort_order) }}" min="0">
                <div class="pf-hint">Lower number appears first</div>
            </div>
        </div>
        <div class="pf-field">
            <label class="pf-label">Description</label>
            <textarea name="description" class="pf-textarea">{{ old('description', $plan->description) }}</textarea>
        </div>
        <div class="pf-field">
            <label class="pf-label" style="display:flex;align-items:center;gap:8px; cursor:pointer;">
                <input type="checkbox" name="is_popular" value="1" {{ old('is_popular', $plan->is_popular) ? 'checked' : '' }} style="width:16px;height:16px;accent-color:var(--brand);">
        ...
        <div style="padding-top:16px; border-top:1px solid var(--border-2); display:flex; gap:12px; align-items:center;">
            <button type="submit" class="pf-submit"><i class="fas fa-save"></i> Update Plan</button>
