...
.interest-card { background: var(--surface); border: 1px solid var(--border); border-radius: 14px; padding: 16px 20px; display: flex; align-items: center; gap: 16px; transition: border-color .2s, box-shadow .2s; }
...
.int-btn { padding: 7px 14px; border-radius: 8px; font-size: .8rem; font-weight: 700; cursor: pointer; border: none; font-family: inherit; display: flex; align-items: center; gap: 5px; transition: all .2s; text-decoration: none; }
...
.int-btn.chat { background: rgba(59,130,246,.1); color: #3b82f6; border: 1px solid rgba(59,130,246,.3); }
.int-btn.chat:hover { background: #3b82f6; color: #fff; }
</style>

<div class="interest-tabs">
    <a href="{{ route('member.interests.received') }}" class="interest-tab-link"><i class="fas fa-inbox"></i> Received</a>
    <a href="{{ route('member.interests.sent') }}" class="interest-tab-link active"><i class="fas fa-paper-plane"></i> Sent</a>
</div>
...
<div class="interests-list">
    ...
    <div class="interest-card">
        <img src="{{ $interest->receiver->photo_url }}" alt="{{ $interest->receiver->name }}" class="interest-card-avatar">
        <div class="interest-card-body">
            <div class="interest-card-name">{{ $interest->receiver->name }}</div>
            <div class="interest-card-meta">
                @if($interest->receiver->profile?->age)
                    <span><i class="fas fa-birthday-cake"></i> {{ $interest->receiver->profile->age }} yrs</span>
                @endif
                @if($interest->receiver->profile?->district)
                    <span><i class="fas fa-map-marker-alt"></i> {{ $interest->receiver->profile->district }}</span>
                @endif
            </div>
            <div class="interest-card-time"><i class="fas fa-clock" style="margin-right:4px;"></i>Sent {{ $interest->created_at->diffForHumans() }}</div>
        </div>
        <div style="display:flex; gap:8px; align-items:center; flex-shrink:0;">
            <a href="{{ route('profile.show', $interest->receiver) }}" class="int-btn view"><i class="fas fa-eye"></i></a>
            @if($interest->status === 'accepted')
                <a href="{{ route('member.messages.show', $interest->receiver) }}" class="int-btn chat"><i class="fas fa-comment"></i> Chat</a>
            @endif
...
<div style="margin-top:24px;">{{ $interests->links() }}</div>
@else
<div style="text-align:center; padding:64px 20px;">
    <i class="fas fa-paper-plane" style="font-size:3rem; color:var(--text4); margin-bottom:16px; display:block;"></i>
    <div style="font-family:'Playfair Display',serif; font-size:1.2rem; color:var(--text2); margin-bottom:8px;">No interests sent yet</div>
    <p style="color:var(--text3); font-size:.9rem;">Browse profiles and send your first interest request.</p>
